...
class WicketGuestPaymentAuth extends WicketGuestPaymentComponent
{
    ...
    public function force_reuse_guest_payment_order($order_id, $checkout): int
    {
        ...
        if (!$user_id) {
            ...
            // HARD FAIL: Guest session active but user not logged in.
            throw new Exception(__('Your session has expired. Please use your guest payment link to try again.', 'wicket-acc'));
        }
    ...
    public function cleanup_after_payment(int $order_id): void
    {
        ...
        if (isset($_COOKIE[self::GUEST_SESSION_COOKIE])) {
            ...
            wp_clear_auth_cookie();
            wp_set_current_user(0); // Ensure global state reflects logout immediately
